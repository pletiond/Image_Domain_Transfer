...
class Solver(object):
    ...
    def __init__(self, model, batch_size=100, pretrain_iter=20000, train_iter=2000, sample_iter=100,
                 ...
                 model_save_path='model', pretrained_model='model/svhn_model-20000', test_model='model/dtn-'):
    ...
    def train(self):
        ...
        with tf.Session(config=self.config) as sess:
            ...
            for step in range(self.train_iter + 1):
                ...
                src_images = svhn_images[i * self.batch_size:(i + 1) * self.batch_size] / 127.5 - 1
                ...
                if (step + 1) % 10 == 0:
                    summary, dl, gl = sess.run([model.summary_op_trg, model.d_loss_trg, model.g_loss_trg], feed_dict)
                    summary_writer.add_summary(summary, step)
    ...
    def eval(self):
        ...
        with tf.Session(config=self.config) as sess:
            ...
            saver.restore(sess, self.test_model+str(model_number))

            print('start sampling..!')
            for i in range(self.sample_iter):
                # train model for source domain S
                batch_images = svhn_images[i * self.batch_size:(i + 1) * self.batch_size] / 127.5 - 1
                ...
                path = os.path.join(self.sample_save_path,
                                    str(model_number)+'-sample-%d-to-%d.png' % (i * self.batch_size, (i + 1) * self.batch_size))
