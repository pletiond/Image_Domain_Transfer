...
class Loader:
    ...
    def load_mnist_data(self):
        ...
        self.mnist_labels = [self.mnist_data[i][0] for i in range(len(self.mnist_data)) if self.mnist_data[i][0] < 5 or self.mnist_data[i][0] ==9]
        self.mnist_images = [self.mnist_data[i][1:] for i in range(len(self.mnist_data)) if self.mnist_data[i][0] < 5 or self.mnist_data[i][0] ==9]
    ...
    def load_fashion_image_data(self):
        ...
        temp.append([self.fashion_filenames[i] for i in range(len(self.fashion_filenames)) if 'Shoes' in self.fashion_filenames[i]])
        ...
        print("Fashion Image(Source domain sample) number :", len(self.fashion_filenames))
    ...
    def eval_fashion_get_next_batch(self, batch_size=128):
        ...
        while len(images) <batch_size:
            ...
            if 'Pullover' in self.fashion_filenames[self.fashion_batch] and len(images) % 2 == 0:
                labels.append(1)
                images.append((cv2.imread(self.fashion_filenames[self.fashion_batch]) / 127.5 - 1))

            elif 'Trouser' in self.fashion_filenames[self.fashion_batch ]  and len(images) % 2 == 1:
                labels.append(2)
                images.append((cv2.imread(self.fashion_filenames[self.fashion_batch ]) / 127.5 - 1))
            ...
            self.fashion_batch += 1
            if self.fashion_batch == self.fashion_num_examples:
                self.fashion_batch = 0
    ...
    def fashion_get_next_batch(self, batch_size=128):
        ...
        while len(images) <batch_size:
            ...
            if 'Coat' in self.fashion_filenames[self.fashion_batch ] :
                labels.append(0)
                images.append((cv2.imread(self.fashion_filenames[self.fashion_batch ]) / 127.5 - 1))
            elif 'Pullover' in self.fashion_filenames[self.fashion_batch]:
                labels.append(1)
                images.append((cv2.imread(self.fashion_filenames[self.fashion_batch]) / 127.5 - 1))

            elif 'Trouser' in self.fashion_filenames[self.fashion_batch ]:
                labels.append(2)
                images.append((cv2.imread(self.fashion_filenames[self.fashion_batch ]) / 127.5 - 1))

            elif 'Shirt' in self.fashion_filenames[self.fashion_batch ]:
                labels.append(3)
                images.append((cv2.imread(self.fashion_filenames[self.fashion_batch ]) / 127.5 - 1))
            elif 'Dress' in self.fashion_filenames[self.fashion_batch ]:
                labels.append(4)
                images.append((cv2.imread(self.fashion_filenames[self.fashion_batch ]) / 127.5 - 1))
            elif 'Shoes' in self.fashion_filenames[self.fashion_batch ]:
                labels.append(5)
                images.append((cv2.imread(self.fashion_filenames[self.fashion_batch ]) / 127.5 - 1))
