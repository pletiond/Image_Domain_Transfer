...
class DTN(object):
    ...
    def build_model(self):
        ...
        if self.mode == 'pretrain':
            self.images = tf.placeholder(tf.float32, [None, 64, 64, 3], 'svhn_images')
            ...
            self.src_images = tf.placeholder(tf.float32, [None, 64, 64, 3], 'svhn_images')
            self.trg_images = tf.placeholder(tf.float32, [None, 64, 64, 1], 'mnist_images')
            ...
            with tf.variable_scope('source_train_op', reuse=False):
                self.d_train_op_src = slim.learning.create_train_op(self.d_loss_src, self.d_optimizer_src,
                                                                    variables_to_train=d_vars)
                self.g_train_op_src = slim.learning.create_train_op(self.g_loss_src, self.g_optimizer_src,
                                                                    variables_to_train=g_vars)
                self.f_train_op_src = slim.learning.create_train_op(self.f_loss_src, self.f_optimizer_src,
